...
export function awardNpcAffinity(state:GameState,npc:VillageNpc,option:NpcOption){if(!option.affinity||hasNpcAffinityReward(state.npcProgress,npc,option))return state;const npcProgress=normalizeNpcProgress(state.npcProgress),key=npcAffinityKey(npc,option);return {...state,npcProgress:{...npcProgress,affinityChoices:[...npcProgress.affinityChoices,key],affinity:{...npcProgress.affinity,[npc.id]:Math.min(100,(npcProgress.affinity[npc.id]||0)+option.affinity)}}}}
