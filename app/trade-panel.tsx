...
export function TradePanel({ trade, gold, stage, escorts, logs, lastEncounter, onDispatch, onSelect, onUpgrade, onToggleAuto }: Props) {
  ...
  return <div className="trade-v21">
    ...
    <section className="panel trade-ledger"><div className="panel-title"><Shield /><h2>最近遭遇戰</h2><span>僅在跑商途中觸發</span></div><output>{lastEncounter}</output><ol>{logs.slice(0, 6).map((log, index) => <li key={index}><span>{String(index + 1).padStart(2, "0")}</span>{log}</li>)}</ol></section>
  </div>;
}
