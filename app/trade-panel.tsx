...
import { cargoCapacity, MAX_CARGO_LEVEL, TRADE_ROUTES, upgradeCost, voyageQuote, type TradeState } from "./trade-engine";

type Props = {
  trade: TradeState; gold: number; stage: number; escorts: number; logs: string[];
...
export function TradePanel({ trade, gold, stage, escorts, logs, onDispatch, onSelect, onUpgrade, onToggleAuto }: Props) {
  ...
  return <div className="trade-v21">
    ...
    <section className="panel trade-ledger"><div className="panel-title"><Package /><h2>商團記事</h2><span>經商與遠征共用日誌</span></div><ol>{logs.slice(0, 6).map((log, index) => <li key={index}><span>{String(index + 1).padStart(2, "0")}</span>{log}</li>)}</ol></section>
  </div>;
}
