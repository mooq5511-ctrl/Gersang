...
export function HeroStatusPanel({busy=false,compact=false,hero,gold,credit,creditXp,creditLevel,weight,xpNeed,allocate,trade,train,select,unequip}:{
  ...
  return <aside className={'hero-personal iron-character'+(compact?' compact':'')} aria-label="主角個人面板">
    ...
    <section className="hp-indicators">
      ...
      <p className="hp-gold">持有信用值：{Math.floor(credit).toLocaleString()}</p>
    ...
    {!compact&&<section className="hp-allocation"><p>剩餘屬性點 <strong>{hero.points}</strong></p>
      {([['str','力量','Str'],['agi','敏捷','Dex'],['vit','體質','Vit'],['intel','智力','Int']] as const).map(([key,name,en])=><div className="hp-stat" key={key}><span>{name} <small>{en}</small></span><small>{hero[key]} ＋ {total[key]-hero[key]}</small><strong>{total[key]}</strong><button aria-label={'增加'+name} disabled={hero.points<=0} onClick={()=>allocate(key)}>＋</button><button aria-label={'增加100點'+name} disabled={hero.points<=0} onClick={()=>allocate(key,100)}>＋100</button></div>)}
