  ...
  return <section className="merchant-bag iron-inventory" aria-label="商隊背包"><h2>行囊 <small>裝備 {items.length}・材料 {materialCount}</small></h2>
    <section className="merchant-material-section" aria-label="戰利品材料">
      ...
      {materialsOpen&&<><label className="merchant-material-search">搜尋材料<input type="search" value={materialQuery} onChange={event=>setMaterialQuery(event.target.value)} placeholder="輸入材料名稱" aria-label="搜尋戰利品材料"/></label><ul className="merchant-material-list merchant-material-detailed-list">{materialEntries.length?filteredMaterialEntries.map(([name,count])=>{
        ...
        return <li className="merchant-material-row merchant-material-detailed-row" key={name} onPointerEnter={event=>showTooltip(ItemTooltipManager.material(name,count,price,sourceEnemies,`/assets/sprites/loot-${artKind}-cute-v1.png`),event)} onPointerLeave={hideTooltip}><span className={'merchant-material-icon material-art material-art-'+artKind}><img src={`/assets/sprites/loot-${artKind}-cute-v1.png`} alt={`${name}圖示`}/></span><div className="merchant-material-copy"><strong>{name}</strong><small>持有 ×{count}・{isAncientCoinBox?'開啟可獲得 1–10 枚新手兌換銅錢・售價 1 兩':`單價 ${price.toLocaleString()} 兩`}</small><small>來源：{sourceLabel}</small><small>地區：{detail?.maps.join('、')||'—'}・用途：{isAncientCoinBox?'開啟寶箱（大吉(作／者／好／帥) 各 0.01%）':'鍛造／交易'}</small></div>{isAncientCoinBox?<span className="merchant-material-actions"><label>開啟數量<input aria-label="古錢箱開啟數量" type="number" min="1" max={count} value={openAmount} onChange={event=>setCoinBoxAmount(Math.min(count,Math.max(1,Math.floor(Number(event.target.value)||1))) )}/></label><button type="button" onClick={()=>openAncientCoinBox(openAmount)}>開啟 ×{openAmount}</button><button type="button" onClick={()=>{if(window.confirm('確定以 1 兩出售「古錢箱」？此操作不會開啟寶箱。'))sellMaterial(name)}}>出售 1 兩</button></span>:<button type="button" onClick={()=>sellMaterial(name)} disabled={price===undefined}>出售 1 件</button>}</li>;
