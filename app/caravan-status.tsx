...
export function CaravanStatus(p:Props) {
  ...
  return <section className="caravan-status" aria-label="主角與商隊狀態">
    <HeroStatusPanel hero={p.hero} gold={p.gold} credit={p.credit} weight={p.weight} xpNeed={p.xpNeed} allocate={p.allocate} trade={p.trade} train={p.trainHero} select={()=>p.select(p.hero.uid)}/>
