...
function MercenaryStatusWindow({unit,power,allocate}:{unit:CaravanMember;power:(unit:CaravanMember)=>number;allocate:(stat:'str'|'agi'|'vit'|'intel')=>void}){
  ...
  return <section className="hero-personal iron-character compact mercenary-status"><div className="mercenary-status-identity"><img src={unit.image} alt={unit.name}/><span><strong>{unit.name}</strong><small>{unit.role} · Lv. {unit.level}</small></span></div>
    ...
    <section className="hp-allocation"><p>剩餘屬性點 <strong>{unit.points}</strong></p><div className="hp-stat"><span>力量 <small>Str</small></span><small>基礎能力</small><strong>{unit.str}</strong><button aria-label="增加力量" disabled={unit.points<=0} onClick={()=>allocate('str')}>＋</button></div><div className="hp-stat"><span>敏捷 <small>Dex</small></span><small>基礎能力</small><strong>{unit.agi}</strong><button aria-label="增加敏捷" disabled={unit.points<=0} onClick={()=>allocate('agi')}>＋</button></div><div className="hp-stat"><span>體質 <small>Vit</small></span><small>基礎能力</small><strong>{unit.vit}</strong><button aria-label="增加體質" disabled={unit.points<=0} onClick={()=>allocate('vit')}>＋</button></div><div className="hp-stat"><span>智力 <small>Int</small></span><small>基礎能力</small><strong>{unit.intel}</strong><button aria-label="增加智力" disabled={unit.points<=0} onClick={()=>allocate('intel')}>＋</button></div><p className="hp-defense">防禦力 <strong>{combat.defense}</strong><small>已包含等級與裝備加成</small></p><div className="hp-vitals"><label className="hp-meter">生命值 HP<span>{vital.hp} / {vital.maxHp}</span><progress className="hp" max={vital.maxHp} value={vital.hp}/></label><label className="hp-meter">魔法值 MP<span>{vital.mp} / {vital.maxMp}</span><progress className="mp" max={vital.maxMp} value={vital.mp}/></label></div><label className="hp-meter hp-exp">EXP<span>{unit.level>=LEVEL_CAP?'已達 Lv.260':`${unit.xp.toLocaleString()} / ${levelData.xpToNext.toLocaleString()}`}</span>{unit.level<LEVEL_CAP&&<progress max={levelData.xpToNext} value={unit.xp}/>}</label></section>
  </section>;
}
