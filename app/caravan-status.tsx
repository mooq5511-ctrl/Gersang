...
export function CaravanStatus(p:Props) {
  ...
  return <section className="caravan-status" aria-label="主角與商隊狀態">
    ...
    {activeWindow==='stats'&&<section className="floating-game-window floating-character" aria-label="角色能力值"><header><strong>{selectedRoster.name}・能力值</strong><button type="button" onClick={()=>setActiveWindow(null)} aria-label="關閉能力值">×</button></header>{selectedRoster.uid==='hero'?<><HeroStatusPanel compact busy={p.busy} hero={p.hero} gold={p.gold} credit={p.credit} weight={p.weight} xpNeed={p.xpNeed} allocate={p.allocate} trade={p.trade} train={p.trainHero} select={()=>p.select(p.hero.uid)} unequip={p.unequipHero}/><AbilityPanel hero={p.hero} allocate={p.allocate}/></>:<MercenaryStatusWindow unit={selectedRoster} power={p.power} allocate={p.allocate}/>}<EquipmentSummary unit={selectedRoster}/></section>}
...
function MercenaryStatusWindow({unit,power,allocate}:{unit:CaravanMember;power:(unit:CaravanMember)=>number;allocate:(stat:'str'|'agi'|'vit'|'intel',amount?:number)=>void}){
  ...
  return <section className="hero-personal iron-character compact mercenary-status"><div className="mercenary-status-identity"><img src={unit.image} alt={unit.name}/><span><strong>{unit.name}</strong><small>{unit.role} · Lv. {unit.level}</small></span></div>
    ...
    <section className="hp-allocation"><p>剩餘屬性點 <strong>{unit.points}</strong></p><div className="hp-stat"><span>力量 <small>Str</small></span><small>基礎能力</small><strong>{unit.str}</strong><button aria-label="增加力量" disabled={unit.points<=0} onClick={()=>allocate('str')}>＋</button><button aria-label="增加100點力量" disabled={unit.points<100} onClick={()=>allocate('str',100)}>＋100</button></div><div className="hp-stat"><span>敏捷 <small>Dex</small></span><small>基礎能力</small><strong>{unit.agi}</strong><button aria-label="增加敏捷" disabled={unit.points<=0} onClick={()=>allocate('agi')}>＋</button><button aria-label="增加100點敏捷" disabled={unit.points<100} onClick={()=>allocate('agi',100)}>＋100</button></div><div className="hp-stat"><span>體質 <small>Vit</small></span><small>基礎能力</small><strong>{unit.vit}</strong><button aria-label="增加體質" disabled={unit.points<=0} onClick={()=>allocate('vit')}>＋</button><button aria-label="增加100點體質" disabled={unit.points<100} onClick={()=>allocate('vit',100)}>＋100</button></div><div className="hp-stat"><span>智力 <small>Int</small></span><small>基礎能力</small><strong>{unit.intel}</strong><button aria-label="增加智力" disabled={unit.points<=0} onClick={()=>allocate('intel')}>＋</button><button aria-label="增加100點智力" disabled={unit.points<100} onClick={()=>allocate('intel',100)}>＋100</button></div><p className="hp-defense">防禦力 <strong>{combat.defense}</strong><small>已包含等級與裝備加成</small></p><div className="hp-vitals"><label className="hp-meter">生命值 HP<span>{vital.hp} / {vital.maxHp}</span><progress className="hp" max={vital.maxHp} value={vital.hp}/></label><label className="hp-meter">魔法值 MP<span>{vital.mp} / {vital.maxMp}</span><progress className="mp" max={vital.maxMp} value={vital.mp}/></label></div><label className="hp-meter hp-exp">EXP<span>{unit.level>=LEVEL_CAP?'已達 Lv.260':`${unit.xp.toLocaleString()} / ${levelData.xpToNext.toLocaleString()}`}</span>{unit.level<LEVEL_CAP&&<progress max={levelData.xpToNext} value={unit.xp}/>}</label></section>
    <footer className="hp-actions"><button type="button" disabled={unit.points<=0} onClick={()=>allocate('str',100)}>力量＋100／剩餘點數</button><small>剩餘不足 100 點時，會自動投入全部可用點數。</small></footer>
  </section>;
}
