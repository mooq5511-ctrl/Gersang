...
type Props = {
  ...
  territory:GuildTerritory;upgradeBuilding:(id:BuildingId)=>void;enhanceEquipment:(itemUid:string)=>void;
...
function MercenaryStatusWindow({unit,power,allocate,promotionItems,promote}:{unit:CaravanMember;power:(unit:CaravanMember)=>number;allocate:(stat:'str'|'agi'|'vit'|'intel',amount?:number)=>void;promotionItems:Readonly<Record<PromotionItemId,number>>;promote:(uid:string,targetTier?:JobTier)=>void}){
  ...
  const promotionPanel=target?<section className="mercenary-promotion" aria-label="傭兵轉職"><strong>進階職業：Tier {target.tier}・{target.name}</strong><small>需要等級：Lv. {target.requiredLevel}（目前 {unit.level}）</small>{target.requiredItems.map(({itemId,quantity})=><small key={itemId}>{itemId}：{promotionItems[itemId] ?? 0}／{quantity}</small>)}<button type="button" disabled={!levelReady||!itemsReady} onClick={()=>promote(unit.uid,target.tier)}>轉職</button></section>:<p className="mercenary-promotion"><small>已達最高階級</small></p>;
