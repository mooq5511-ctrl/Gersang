...
export function BattleArena({state,hero}:{state:DungeonState;hero:CaravanMember & {nation?:string}}){
 ...
 const v=vitalStats(hero),monster=DUNGEONS[state.key],shieldActive=monster.name==='狂風阿魯塔'&&(state.enemyShieldUntil||0)>Date.now(),shieldSeconds=shieldActive?Math.max(1,Math.ceil(((state.enemyShieldUntil||0)-Date.now())/1000)):0,fearActive=(state.fearUntil||0)>Date.now(),fearSeconds=fearActive?Math.max(1,Math.ceil(((state.fearUntil||0)-Date.now())/1000)):0,skillInfo=monster.name==='狂風阿魯塔'?'白虎盾：消耗 500 MP，防禦 +30%，持續 3 秒，冷卻 60 秒。\n風碎：消耗 500 MP，無視防禦造成 2000 傷害，冷卻 30 秒。':monster.name==='黃金海星'?'恢復術：恢復自身 150% ATK 生命，並獲得每秒 0.5% 最大生命值的持續療癒，持續 3 秒，冷卻 60 秒。\n火焰燎原（被動）：每次攻擊有 30% 機率觸發，全體造成 180% 火屬性魔法傷害並施加灼燒 6 秒（每秒 15% ATK，可疊 3 層）；灼燒目標暴擊率 +15%。\n詛咒：攻擊力最高目標 ATK -25%、DEF -30%，持續 8 秒；受詛咒目標承傷 +15%，冷卻 60 秒。':('skill' in monster&&monster.skill?monster.skill:'');
...
<span className="impact-seal impact-sprite impact-sprite-enemy" data-sprite="enemy" data-monster={monster.name}><img src={monsterArt[monster.name]||'/assets/sprites/enemy-idle.png'} alt=""/>{shieldActive&&<span className="enemy-status-icon enemy-status-shield" title={`白虎盾・防禦 +30%・剩餘 ${shieldSeconds} 秒`} aria-label={`白虎盾，防禦提升 30%，剩餘 ${shieldSeconds} 秒`}><Shield size={16}/><small>{shieldSeconds}s</small></span>}{fearActive&&<span className="enemy-status-icon" title={`恐懼・防禦 -20%・剩餘 ${fearSeconds} 秒`} aria-label={`恐懼，防禦降低 20%，剩餘 ${fearSeconds} 秒`}><Frown size={16}/><small>{fearSeconds}s</small></span>}</span><h3>{monster.name}</h3><p>Lv.{monster.level} · 攻擊 {monster.atk}</p>{skillInfo&&<><p className="enemy-skill-label" title={skillInfo}>技能：{skillInfo.split('：')[0]} ⓘ</p><details className="enemy-skill-details"><summary>技能資訊</summary>{skillInfo.split('\n').map((line,index)=><span key={index}>{line}</span>)}</details></>}
