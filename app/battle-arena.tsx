...
function UnitSprite({unit,name,level,image,status,boss=false,tiger=false,theater=false}:{unit:RealtimeBattleUnitState;name:string;level:number;image:string;status?:'shield'|'fear';boss?:boolean;tiger?:boolean;theater?:boolean}){
 const resolvedImage=tiger?battleMonsterImage('狂虎')+'?v=20260910':image,crop=BATTLE_MONSTER_CROP[name];
 const point=theater?null:gridToPixel(unit.side,unit.position.row,unit.position.col),hp=Math.max(0,unit.hp),hpRate=Math.max(0,Math.min(100,hp/unit.maxHp*100));
 return <article className={'realtime-unit '+unit.side+(boss?' realtime-unit-boss':'')+(theater?' realtime-unit-theater':'')+(hp<=0?' realtime-unit-dead':'')} data-unit-id={unit.id} style={point?{left:point.x,top:point.y}:undefined} aria-label={`${boss?'首領 ':''}${name}，生命 ${hp} / ${unit.maxHp}`}>
...
export function BattleArena({state,hero,party}:{state:DungeonState;hero:CaravanMember & {nation?:string};party:CaravanMember[]}){
 ...
 const shieldActive=monster.name==='狂風阿魯塔'&&effectUntil(enemies[0],'bossShield')>(state.realtime?.timeMs||0),fearActive=effectUntil(enemies[0],'amaterasuFearDefense')>(state.realtime?.timeMs||0);
 return <div className="realtime-stage-scroll" aria-label={enemyCount?`${enemyCount} 名敵軍在上、${players.length||party.length} 名我方傭兵在下的即時自動戰鬥`:'即時自動戰鬥場景，等待遭遇'}><div className={'realtime-battle-stage realtime-battle-theater'+(state.status==='fighting'?' realtime-stage-fighting':'')} ref={root}>
  <section className="battle-theater-camp battle-theater-enemy"><header className="realtime-camp-label enemy"><strong>{boss?'首領・':''}{monster.name}</strong><span>{enemySlots.filter(unit=>unit.hp>0).length}／{enemyCount} 存活</span></header><div className={'battle-theater-enemy-units'+(boss?' battle-theater-boss':'')}>
   {enemySlots.map(unit=><UnitSprite key={unit.id} unit={unit} name={monster.name} level={monster.level} image={battleMonsterImage(monster.name,state.key)} status={shieldActive?'shield':fearActive?'fear':undefined} boss={boss} tiger={state.key==='e_white_tiger_fierce_tiger'} theater/>)}</div></section>
  ...
  <section className="battle-theater-camp battle-theater-player"><header className="realtime-camp-label player"><strong>我方商隊</strong><span>{players.filter(unit=>unit.hp>0).length||party.filter(unit=>(unit.hp||0)>0).length}／{party.length} 存活</span></header><div className="battle-theater-player-units">
   {players.length?players.map((unit,index)=>{const member=memberById.get(unit.id)||party[index];return member?<UnitSprite key={unit.id} unit={unit} name={member.name} level={member.level} image={member.image} theater/>:null}):party.slice(0,12).map((member,index)=><UnitSprite key={member.uid} unit={{id:member.uid,side:'player',hp:member.hp||0,maxHp:member.maxHp||1,atk:1,def:0,attackInterval:1.5,cooldown:0,mp:index===0?v.mp:member.mp||0,position:{row:Math.floor(index/4),col:index%4}}} name={member.name} level={member.level} image={member.image} theater/>)}</div></section>
 </div></div>;
}
