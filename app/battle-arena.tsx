...
import {BATTLE_MONSTER_CROP,battleMonsterImage,battleMonsterInjuredImage} from './battle-visual-data';
...
function UnitSprite({unit,name,level,image,injuredImage,timeMs,boss=false,tiger=false,theater=false,formationPosition,healthGroup,hideHealthHud=false}:{unit:RealtimeBattleUnitState;name:string;level:number;image:string;injuredImage?:string;timeMs:number;boss?:boolean;tiger?:boolean;theater?:boolean;formationPosition?:CaravanMember['position'];healthGroup?:{hp:number;maxHp:number;count:number};hideHealthHud?:boolean}){
 const normalImage=tiger?battleMonsterImage('狂虎')+'?v=20260910':image,crop=BATTLE_MONSTER_CROP[name];
 const point=theater?null:gridToPixel(unit.side,unit.position.row,unit.position.col),hp=Math.max(0,unit.hp),hpRate=Math.max(0,Math.min(100,hp/unit.maxHp*100)),healthHp=Math.max(0,healthGroup?.hp??hp),healthMaxHp=Math.max(1,healthGroup?.maxHp??unit.maxHp),healthRate=Math.max(0,Math.min(100,healthHp/healthMaxHp*100)),healthLabel=name+(healthGroup&&healthGroup.count>1?` ×${healthGroup.count}`:''),statuses=battleUnitStatuses(unit,timeMs),formationClass=formationPosition==='前排'?'front':formationPosition==='中排'?'middle':formationPosition==='後排'?'rear':'';
 const [tailHpRate,setTailHpRate]=useState(healthRate),[damagePopups,setDamagePopups]=useState<{id:number;amount:number;type:'damage'|'heal'}[]>([]),[hitPulse,setHitPulse]=useState(0),[injured,setInjured]=useState(false),previousHp=useRef(healthHp),previousHpRate=useRef(healthRate),tailTimer=useRef<ReturnType<typeof setTimeout>|null>(null),injuredTimer=useRef<ReturnType<typeof setTimeout>|null>(null),popupId=useRef(0);
 useEffect(()=>{const delta=healthHp-previousHp.current;if(delta!==0){const id=++popupId.current;setDamagePopups(previous=>[...previous.slice(-5),{id,amount:Math.abs(delta),type:delta>0?'heal':'damage'}]);if(delta<0)setHitPulse(id)}previousHp.current=healthHp},[healthHp]);
 useEffect(()=>{if(hitPulse<=0||!injuredImage||unit.side!=='enemy')return;setInjured(true);if(injuredTimer.current)clearTimeout(injuredTimer.current);injuredTimer.current=setTimeout(()=>setInjured(false),650);return()=>{if(injuredTimer.current)clearTimeout(injuredTimer.current)}},[hitPulse,injuredImage,unit.side]);
 useEffect(()=>{if(tailTimer.current)clearTimeout(tailTimer.current);if(healthRate>=previousHpRate.current)setTailHpRate(healthRate);else tailTimer.current=setTimeout(()=>setTailHpRate(healthRate),380);previousHpRate.current=healthRate;return()=>{if(tailTimer.current)clearTimeout(tailTimer.current)}},[healthRate]);
 return <article className={'realtime-unit '+unit.side+(boss?' realtime-unit-boss':'')+(theater?' realtime-unit-theater':'')+(hp<=0?' realtime-unit-dead':'')} data-unit-id={unit.id} style={point?{left:point.x,top:point.y}:undefined} aria-label={`${boss?'首領 ':''}${name}${formationPosition?`，${formationPosition}`:''}，生命 ${hp} / ${unit.maxHp}`}>
  {!hideHealthHud&&unit.side==='enemy'?<div className={'realtime-health-hud'+(boss?' realtime-health-hud-boss':'')} aria-label={`${healthLabel}，生命 ${healthHp} / ${healthMaxHp}`}>
   <span className="realtime-health-emblem" aria-hidden="true"><img src={boss?'/assets/ui/boss-emblem.png':'/assets/ui/monster-sword-emblem.png'} alt=""/></span>
   <div className="realtime-health-main">
    <div className="realtime-health-heading"><strong>{healthLabel}</strong><small>Lv.{level}</small></div>
    <div className="realtime-unit-bars"><span className="realtime-hp" style={{'--hp-now':healthRate+'%','--hp-tail':tailHpRate+'%'} as CSSProperties}><i className="realtime-hp-tail" style={{width:tailHpRate+'%'}}/><i className="realtime-hp-now" style={{width:healthRate+'%'}}/>{hitPulse>0&&<b key={hitPulse} className="realtime-hp-impact" aria-hidden="true"/>}<b className="realtime-hp-value">{healthHp.toLocaleString()} / {healthMaxHp.toLocaleString()}</b></span><span className="realtime-mp"><i style={{width:Math.max(0,Math.min(100,unit.mp))+'%'}}/></span></div>
   </div>
  </div>:!hideHealthHud?<div className="realtime-unit-bars"><span className="realtime-hp" style={{'--hp-now':hpRate+'%','--hp-tail':tailHpRate+'%'} as CSSProperties}><i className="realtime-hp-tail" style={{width:tailHpRate+'%'}}/><i className="realtime-hp-now" style={{width:hpRate+'%'}}/>{hitPulse>0&&<b key={hitPulse} className="realtime-hp-impact" aria-hidden="true"/>}<b className="realtime-hp-value">{hp.toLocaleString()} / {unit.maxHp.toLocaleString()}</b></span><span className="realtime-mp"><i style={{width:Math.max(0,Math.min(100,unit.mp))+'%'}}/></span></div>:null}
  {damagePopups.map((popup,index)=><b key={popup.id} style={{'--damage-offset':`${((index%3)-1)*30}px`} as CSSProperties} className={popup.type==='heal'?'realtime-floating-damage realtime-floating-heal':'realtime-floating-damage '+(unit.side==='enemy'?'enemy-damage':'player-damage')} onAnimationEnd={()=>setDamagePopups(previous=>previous.filter(item=>item.id!==popup.id))}>{popup.type==='heal'?'+':'-'}{popup.amount.toLocaleString()}</b>)}
  <span className={'realtime-portrait'+(injured&&injuredImage?' realtime-portrait-injured':'')} style={{backgroundImage:`url(${injured&&injuredImage?injuredImage:normalImage})`,backgroundSize:crop?.size||'contain',backgroundRepeat:'no-repeat',backgroundPosition:crop?.position||'center'}}>{!crop&&<img key={injured&&injuredImage?injuredImage:normalImage} src={injured&&injuredImage?injuredImage:normalImage} alt={tiger?'狂虎':''} onError={event=>{event.currentTarget.style.display='none'}}/>}</span>
...
export function BattleArena({state,hero,party,mapTheme}:{state:DungeonState;hero:CaravanMember & {nation?:string};party:CaravanMember[];mapTheme?:string}){
 ...
 const monster=DUNGEONS[state.key],boss=state.key==='e_white_tiger_fierce_tiger'||isBossMonster(monster.name),sceneTheme=mapTheme&&['field','lake','sea','forest'].includes(mapTheme)?mapTheme:'default',enemyImage=battleMonsterImage(monster.name,state.key),enemyInjuredImage=battleMonsterInjuredImage(monster.name,state.key),v=vitalStats(hero),players=state.realtime?.players||[],enemies=state.realtime?.enemies||[],memberById=new Map(party.map(member=>[member.uid,member]));
 ...
 const enemyHealthOwnerId=enemySlots.find(unit=>unit.hp>0)?.id||enemySlots[0]?.id,enemyHealthGroup=!boss?{hp:enemySlots.reduce((total,unit)=>total+Math.max(0,unit.hp),0),maxHp:enemySlots.reduce((total,unit)=>total+Math.max(0,unit.maxHp),0),count:enemySlots.filter(unit=>unit.hp>0).length}:undefined;
 ...
 return <div className="battle-viewport" ref={viewport} aria-label="完整戰鬥畫面"><div className="realtime-stage-scroll" aria-label={enemyCount?`${enemyCount} 名敵軍在上、${players.length||party.length} 名我方傭兵在下的即時自動戰鬥`:'即時自動戰鬥場景，等待遭遇'}><div className={'realtime-battle-stage realtime-battle-theater realtime-battle-theme-'+sceneTheme+(state.status==='fighting'?' realtime-stage-fighting':'')} style={stageStyle} ref={root}>
  <section className="battle-theater-camp battle-theater-enemy"><header className="realtime-camp-label enemy"><strong>{boss?'首領・':''}{monster.name}</strong><span>{enemySlots.filter(unit=>unit.hp>0).length}／{enemyCount} 存活</span></header><div className={'battle-theater-enemy-units'+(boss?' battle-theater-boss':'')}>
   {enemySlots.filter(unit=>boss||unit.id===enemyHealthOwnerId).map(unit=><UnitSprite key={unit.id} unit={unit} name={monster.name} level={monster.level} image={enemyImage} injuredImage={enemyInjuredImage} timeMs={timeMs} boss={boss} tiger={state.key==='e_white_tiger_fierce_tiger'} theater healthGroup={unit.id===enemyHealthOwnerId?enemyHealthGroup:undefined} hideHealthHud={!boss&&unit.id!==enemyHealthOwnerId}/>)}</div></section>
