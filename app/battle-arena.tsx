...
function UnitSprite({unit,name,level,image,timeMs,boss=false,tiger=false,theater=false}:{unit:RealtimeBattleUnitState;name:string;level:number;image:string;timeMs:number;boss?:boolean;tiger?:boolean;theater?:boolean}){
 const resolvedImage=tiger?battleMonsterImage('狂虎')+'?v=20260910':image,crop=BATTLE_MONSTER_CROP[name];
 const point=theater?null:gridToPixel(unit.side,unit.position.row,unit.position.col),hp=Math.max(0,unit.hp),hpRate=Math.max(0,Math.min(100,hp/unit.maxHp*100)),statuses=battleUnitStatuses(unit,timeMs);
