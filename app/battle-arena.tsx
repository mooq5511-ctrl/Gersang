...
export function BattleArena({state,hero,party}:{state:DungeonState;hero:CaravanMember & {nation?:string};party:CaravanMember[]}){
 ...
 return <div className="realtime-stage-scroll" aria-label={`${boss?'首領 1 隻':'12 對 12'} 左右對峙即時自動戰鬥`}><div className={'realtime-battle-stage'+(state.status==='fighting'?' realtime-stage-fighting':'')} ref={root}>
