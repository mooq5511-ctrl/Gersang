...
export type DungeonState={status:'idle'|'fighting'|'respawning'|'recovering';zone?:ZoneId;key:DungeonKey;enemyHp:number;normalAt:number;skillAt:number;spawnAt:number;stamp:number;pauseAt?:number;logs:string[];serial:number};
...
export const freshDungeon=():DungeonState=>({status:'idle',zone:'hanyang',key:'thug',enemyHp:150,normalAt:0,skillAt:0,spawnAt:0,stamp:0,logs:[],serial:0});
...
export function teleportDungeon(old:DungeonState,level:number,power:number,now:number,id:string):DungeonState{
 const zone=WORLD_ZONES.find(entry=>entry.id===id);
 if(!zone||!zoneUnlocked(zone,level,power)||old.status==='recovering')return old;
 return {...old,zone:zone.id,key:zone.enemy,enemyHp:DUNGEONS[zone.enemy].hp,status:'fighting',stamp:now,
...
export function dungeonStep(old:DungeonState,hero:DungeonHero,action:'tick'|'start'|'normal'|'skill'|'retreat',now:number,key:DungeonKey=old.key,roll=.99,choice=0):{state:DungeonState;hp:number;mp:number;reward:null|{xp:number;gold:number;loot:string|null}}{
 ...
 const recover=()=>{state.status='recovering';state.zone='hanyang';state.key='thug';state.enemyHp=DUNGEONS.thug.hp;state.spawnAt=0;log('商隊不幸全滅，已被熱心商旅送回漢陽療傷...')};
