...
export type DungeonState={events?:BattleEvent[];eventSerial?:number;spawnSerial?:number;innHealAt?:number;enemyShieldAt?:number;enemyShatterAt?:number;enemyShieldUntil?:number;enemyRegenUntil?:number;enemyFlameAt?:number;enemyCurseAt?:number;burnUntil?:number;burnStacks?:number;cursedUid?:string;curseUntil?:number;/** 指定狩獵時鎖定下一次重生的目標。 */lockedEnemyKey?:DungeonKey;status:'idle'|'fighting'|'respawning'|'recovering';phase?:'接敵'|'交戰';distance?:number;damageCursor?:number;zone?:ZoneId;key:DungeonKey;enemyHp:number;normalAt:number;skillAt:number;spawnAt:number;stamp:number;pauseAt?:number;logs:string[];serial:number};
...
export function dungeonStep(old:DungeonState,hero:DungeonHero,action:'tick'|'start'|'normal'|'skill'|'retreat',now:number,key:DungeonKey=old.key,roll=.99,choice=0,spawnRoll=0,retaliationRoll=0,party:DungeonPartyMember[]=[],passiveDamage=0,materialRolls:number[]=[1,1,1],autoSkill=false):{state:DungeonState;hp:number;mp:number;party:DungeonPartyMember[];reward:null|{xp:number;gold:number;loot:string|null;materials:string[]}}{
 ...
 const enemyCombatMultiplier=()=>(state.zone==='miasma-forest'||state.key.startsWith('e_white_tiger_'))&&members.filter(member=>member.uid!=='hero').length>5?2:1;
 const enemy=()=>{const base=DUNGEONS[state.key],multiplier=enemyCombatMultiplier();if(multiplier===1)return base;return {...base,hp:base.hp*multiplier,mp:base.mp*multiplier,atk:base.atk*multiplier,dex:base.dex*multiplier}};
 ...
 const hit=(skill=false)=>{
  ...
  // 蛇龍出水：固定基礎傷害 5,000，再加上全體已僱用傭兵智力總和 × 1.5；不受一般 ATK 或武器倍率影響。
  const attack=state.cursedUid==='hero'&&now<(state.curseUntil||0)?hero.attack*.75:hero.attack;
  let damage=Math.max(1,Math.floor(skill?5000+hero.mercenaryIntelligence*1.5:hero.str*2+attack));
