...
export function GersangArchive(){
 ...
 const entries=useMemo<Entry[]>(()=>{
   ...
   return [...base,...official,...mythic,...gems,...materials].filter((entry,index,all)=>all.findIndex(other=>other.category===entry.category&&other.name===entry.name)===index);
 },[]);
 ...
 return <section className="panel equipment-codex" aria-label="裝備圖鑑">
   ...
   <div className="equipment-codex-toolbar"><label><Search/><Input value={query} onChange={event=>setQuery(event.target.value)} placeholder="搜尋名稱、部位、屬性或效果" aria-label="搜尋裝備圖鑑"/></label><div role="group" aria-label="圖鑑分類">{(['全部','裝備','道具','材料'] as Category[]).map(item=><button type="button" key={item} aria-pressed={category===item} onClick={()=>setCategory(item)}>{item}</button>)}</div></div>
