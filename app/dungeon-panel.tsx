...
export function DungeonPanel({state,hero,party,dps=0,act,autoSkill,toggleAutoSkill,autoPotion,healingPotions,medicineStock,onAutoPotionChange,battleLogs,clearBattleLogs,mapName,mapRegion,medicineQuickbar}:{state:DungeonState;mp:number;hero:CaravanMember;party:CaravanMember[];dps?:number;act:(action:DungeonAction,key?:DungeonKey)=>void;autoSkill:boolean;toggleAutoSkill:()=>void;autoPotion:AutoPotionSettings;healingPotions:HealingPotion[];medicineStock:Record<string,number>;onAutoPotionChange:(change:Partial<AutoPotionSettings>)=>void;battleLogs:BattleLogEntry[];clearBattleLogs:()=>void;mapName?:string;mapRegion?:string;medicineQuickbar?:ReactNode}){
 ...
 <div className="dungeon-auto-potion"><label><input type="checkbox" checked={autoPotion.enabled} onChange={event=>onAutoPotionChange({enabled:event.target.checked})}/><strong>Auto Potion</strong></label><label>使用食物／藥品<select aria-label="Auto Potion 使用食物或藥品" value={healingPotions.some(potion=>potion.id===autoPotion.medicineId)?autoPotion.medicineId||'':''} onChange={event=>onAutoPotionChange({medicineId:event.target.value||null})}><option value="">{healingPotions.length?'選擇食物或藥品':'目前沒有可恢復 HP 的物品'}</option>{healingPotions.map(potion=><option key={potion.id} value={potion.id}>{potion.name} ×{Math.max(0,Math.floor(medicineStock[potion.id]||0))}</option>)}</select>{autoPotion.medicineId&&<small className="dungeon-auto-potion-quantity">即時持有 ×{selectedPotionQuantity}</small>}</label><label title="主角或任一名出戰傭兵剩餘 HP 低於此比例時，會自動使用所選食物或藥品">剩餘血量<select aria-label="Auto Potion 剩餘血量門檻" value={autoPotion.threshold} onChange={event=>onAutoPotionChange({threshold:Number(event.target.value) as AutoPotionSettings['threshold']})}>{[30,40,50,60,70,80,90].map(threshold=><option key={threshold} value={threshold}>剩餘 {threshold}% 以下</option>)}</select></label></div>
