...
import type {DungeonAction} from './game-battle-actions';
...
export function DungeonPanel({state,hero,party,dps=0,act,autoSkill,toggleAutoSkill,autoPotion,healingPotions,onAutoPotionChange,battleLogs,clearBattleLogs,mapName,mapRegion,medicineQuickbar}:{state:DungeonState;mp:number;hero:CaravanMember;party:CaravanMember[];dps?:number;act:(action:DungeonAction,key?:DungeonKey)=>void;autoSkill:boolean;toggleAutoSkill:()=>void;autoPotion:AutoPotionSettings;healingPotions:HealingPotion[];onAutoPotionChange:(change:Partial<AutoPotionSettings>)=>void;battleLogs:BattleLogEntry[];clearBattleLogs:()=>void;mapName?:string;mapRegion?:string;medicineQuickbar?:ReactNode}){
 ...
 <header className="dungeon-command-header"><div><small>{mapRegion ? mapRegion+' · 戰鬥地圖同步' : zone.mood}</small><h2>{mapName || zone.name}</h2></div><div className="dungeon-command-controls"><span className={'dungeon-status dungeon-status-'+state.status}>{{idle:'整裝待發',fighting:'交鋒中',respawning:'等待下一隻',recovering:'漢陽療傷中'}[state.status]}</span><button type="button" role="switch" aria-checked={state.autoHunt===true} aria-label="Auto Hunt" title="勝利後自動尋找下一批怪物" className={'dungeon-auto-hunt-toggle'+(state.autoHunt?' enabled':'')} disabled={state.status==='recovering'} onClick={()=>act('toggle-auto-hunt',state.key)}><span>Auto Hunt</span><b>{state.autoHunt?'ON':'OFF'}</b></button></div></header>
 ...
 <div className="dungeon-auto-potion"><label><input type="checkbox" checked={autoPotion.enabled} onChange={event=>onAutoPotionChange({enabled:event.target.checked})}/><strong>Auto Potion</strong></label><label>使用物品<select aria-label="Auto Potion 使用物品" value={healingPotions.some(potion=>potion.id===autoPotion.medicineId)?autoPotion.medicineId||'':''} onChange={event=>onAutoPotionChange({medicineId:event.target.value||null})}><option value="">{healingPotions.length?'選擇補血道具':'目前沒有補血藥'}</option>{healingPotions.map(potion=><option key={potion.id} value={potion.id}>{potion.name} ×{potion.quantity}</option>)}</select></label><label>觸發血量<select aria-label="Auto Potion 觸發血量" value={autoPotion.threshold} onChange={event=>onAutoPotionChange({threshold:Number(event.target.value) as AutoPotionSettings['threshold']})}>{[30,40,50,60,70,80,90].map(threshold=><option key={threshold} value={threshold}>{threshold}%</option>)}</select></label></div>
 <div className="dungeon-actions realtime-actions"><span>{state.status==='idle'?(state.autoHunt?'自動狩獵已開啟；開始狩獵後會自動接續下一場。':'開始狩獵會完成本場；開啟右上「自動狩獵」才會接續下一場。'):state.autoHunt?'每名角色依自己的攻速冷卻自動鎖敵；勝利後會自動尋找下一批怪物。':'每名角色依自己的攻速冷卻自動鎖敵；本場結束後會停止。'}</span><button type="button" disabled={state.status==='recovering'} onClick={()=>act(state.status==='fighting'||state.status==='respawning'?'stop':'start',state.key)}>{state.status==='recovering'?'客棧療傷中':state.status==='idle'?'開始狩獵':'停止狩獵'}</button><button type="button" disabled={!active&&state.status!=='respawning'} onClick={()=>act('retreat')}>{state.status==='recovering'?'客棧療傷中':'撤退至客棧'}</button></div>
 ...
 <details className="battle-log-panel" open><summary>Battle Log · 最近 {battleLogs.length} / 100 筆</summary><button type="button" onClick={clearBattleLogs} disabled={!battleLogs.length}>清除紀錄</button>{battleLogs.length?<ol>{battleLogs.map(entry=><li key={entry.id} className={'battle-log-'+entry.category}>{entry.message}</li>)}</ol>:<p>尚無戰鬥紀錄。</p>}</details>
