...
import { heroNationProfiles, medicineCatalog, monsterDungeonKeys, SHOP_QUALITY } from "./game-config";
...
import { allocateAttributeAction, cyclePositionAction, recruitMerchantAction, storeMercenaryAction, toggleActiveAction, withdrawMercenaryAction } from "./game-squad-actions";
import { applyAutoMedicineAction, buyMaterialAction, buyMedicineAction, consumeMedicineAction, depositWarehouseItemAction, equipInventoryItemAction, forgeThunderItemAction, forgeVillageWeaponAction, openAncientCoinBoxAction, purchaseEquipmentAction, sellAllInventoryEquipmentAction, sellAllMaterialsAction, sellInventoryEquipmentAction, sellMaterialAction, socketGemAction, unequipInventoryItemAction, withdrawWarehouseItemAction } from "./game-inventory-actions";
...
export default function GameV15() {
  ...
  function enterCharacter(slot: number) {
    ...
    try {
      ...
      next = settleCurrentGame(next, { now, roll: .99, choice: 0, spawnRoll: 0, retaliationRoll: 0, materialRolls: [1, 1, 1] });
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value)} className="game-tabs">
        ...
        <TabsContent value="battle" className="tab-panel">
          ...
          <section className="panel battle-map-panel">
            ...
            <header className="battle-world-map-header"><div><small>東方商路</small><h2>世界地圖</h2><p>選擇已解鎖的區域後，即可指定怪物並持續戰鬥。</p></div><span>目前：{currentMap.name}</span></header>
            ...
            <div className="battle-world-map" aria-label="世界地圖戰鬥區域">
              ...
              {battleMaps.map((map) => {
                ...
                return <button key={map.id} style={{'--map-x':x+'%','--map-y':y+'%'} as React.CSSProperties} className={'battle-map-node map-theme-'+map.theme+' '+(currentMap.id === map.id ? "active " : "") + (unlocked ? "" : "locked")} disabled={!unlocked} onClick={() => selectBattleMap(map.id)}>
