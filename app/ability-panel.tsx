...
export function AbilityPanel({ hero, allocate }: { hero: CaravanMember; allocate: (stat: StatKey) => void }) {
  ...
  return <section className="ability-panel" aria-label="主角能力樹">
    <header className="ability-header"><div><small>Ability Archive · 角色成長</small><h2>主角能力樹</h2><p>把能力點投入節點，立即套用到現有四圍與戰力計算。</p></div><div className="ability-points"><div className="ability-point-slot"><img src="/game-assets/ability/AbilityPointSlot-0.png" alt=""/><strong>{hero.points}</strong></div><span>可用能力點</span></div></header>
