...
export function CityHall({
  ...
  return <section className="city-hall-board" aria-label="市政廳委託中心">
    ...
    <section className="city-hall-section"><h3>可接取委託</h3><div className="city-hall-commission-grid">{available.map((commission) => <article className="city-hall-commission-card" key={commission.id}><header><span>{commission.category}</span><strong>{commission.name}</strong></header><p>{commission.description}</p><small>目標：{commission.objectiveLabel} {commission.target} 次</small><small>地點：{commission.locationLabel}</small><em>操作：{commission.actionHint}</em><em>獎勵：{rewardText(commission)}</em><Button size="sm" onClick={() => onAccept(commission.id)} disabled={active.length >= activeLimit}>接取委託</Button></article>)}{available.length === 0 && <p className="city-hall-empty">目前沒有可接取委託，請使用委託刷新券。</p>}</div></section>

    <section className="city-hall-section"><h3>進行中委託</h3><div className="city-hall-active-list">{active.map(({ entry, commission, progress }) => { const ready = progress >= commission.target; return <article className={ready ? "city-hall-active-card ready" : "city-hall-active-card"} key={entry.id}><div className="city-hall-active-copy"><span>{commission.category}・{commission.locationLabel}</span><strong>{commission.name}</strong><p>{commission.actionHint}</p></div><div className="city-hall-active-progress"><span>{commission.objectiveLabel}：{Math.min(progress, commission.target)} / {commission.target}</span><Progress value={Math.min(100, progress / commission.target * 100)} /><small>{ready ? "條件已完成，可領取獎勵" : `還需要${Math.max(0, commission.target - progress)} 次，完成後回到市政廳`}</small></div><div className="city-hall-active-actions"><Button size="sm" variant={ready ? "default" : "outline"} disabled={!ready} onClick={() => onClaim(entry.id)}>{ready ? <CheckCircle2 /> : <XCircle />}{ready ? "領取獎勵" : "進行中"}</Button><Button size="sm" variant="ghost" onClick={() => onAbandon(entry.id)}>放棄</Button></div></article>; })}{active.length === 0 && <p className="city-hall-empty">尚未接取市政廳委託。</p>}</div></section>
